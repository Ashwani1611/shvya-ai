...
class SupportIntegrationTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        config.email_issue=cls.issue
        config.email_priority=cls.priority
        config.save()
    ...
    def test_pipeline_and_issue_relationships_are_validated(self):
        with self.assertRaises(Http404):
            self.create(pipeline_id=self.foreign_pipeline.pk)
        ...
        with self.assertRaises(Http404):
            self.create(issue_id=other_issue.pk)
    ...
    def test_organization_and_own_user_visibility(self):
        ...
        with self.assertRaises(Http404):
            services.reply(ticket_id=ticket.pk,actor=self.agent,body='Unauthorized')
    ...
    def test_staff_reply_default_assignment_and_internal_note(self):
        config=SupportSettings.load()
        config.auto_assign_first_reply=True
        config.save()
        ticket=self.create()
        ...
        self.assertEqual(ticket.status.key,'answered')
        self.assertEqual(ticket.assignee,self.staff)
        self.assertIsNotNone(ticket.first_staff_reply_at)
        with self.assertRaises(PermissionDenied):
            services.reply(ticket_id=ticket.pk,actor=self.user,body='Private?',internal=True)
    ...
    def test_custom_status_family_and_system_key_protection(self):
        ...
        system=TicketStatus.objects.get(key='open')
        system.behavior='closed'
    ...
    def test_customer_cannot_update_properties_or_other_org(self):
        ticket=self.create()
        with self.assertRaises(Http404):
            services.update_ticket(ticket_id=ticket.pk,actor=self.outsider,status_id=ticket.status_id)
        with self.assertRaises(PermissionDenied):
            services.update_ticket(ticket_id=ticket.pk,actor=self.user,change_assignee=True,assignee_id=self.staff.pk)
        with self.assertRaises(PermissionDenied):
            services.update_ticket(ticket_id=ticket.pk,actor=self.user,status_id=TicketStatus.objects.get(key='answered').pk)

    def test_optimistic_update_conflict(self):
        ticket=self.create()
        original=ticket.version
    ...
    def test_creation_and_reply_idempotency(self):
        key=uuid.uuid4()
        one=self.create(client_key=key)
        two=self.create(client_key=key)
        self.assertEqual(one.pk,two.pk)
        key=uuid.uuid4()
        for _ in range(2):
            services.reply(ticket_id=one.pk,actor=self.user,body='Same submit',client_key=key)
    ...
    def test_required_and_internal_custom_fields(self):
        ...
        with self.assertRaises(ValidationError):
            self.create(custom={'steps':'X','resolution':'cannot write this'})
        with self.assertRaises(ValidationError):
            services.update_ticket(ticket_id=ticket.pk,actor=self.user,status_id=TicketStatus.objects.get(key='closed').pk)
        services.update_ticket(ticket_id=ticket.pk,actor=self.staff,custom={'resolution':'Reviewed'})
        ticket.refresh_from_db()
        self.assertNotIn(('Internal resolution','Reviewed'),public_custom(ticket.custom_values))
    ...
    def test_private_upload_exact_count_limit(self):
        ...
        with self.assertRaises(ValidationError):
            self.create(files=[SimpleUploadedFile('a.txt',b'a'),SimpleUploadedFile('b.txt',b'b')])
        with self.assertRaises(ValueError):
            PrivateSupportStorage().url('never-public')
    ...
    def test_public_share_never_renders_or_downloads_internal_note(self):
        ...
        with self.assertRaises(Http404):
            views.shared_attachment(self.request(),token=raw,attachment_id=attachment.pk)

    def test_shared_write_requires_csrf_through_http_middleware(self):
        ticket=self.create()
        _,raw=services.issue_share(actor=self.user,ticket_id=ticket.pk,label='Viewer',can_reply=True)
    ...
    def test_merge_preserves_internal_work_and_resolves_old_reply(self):
        primary=self.create()
        source=self.create(subject='Second ticket')
        ...
        self.assertEqual(source.merged_into,primary)
        self.assertEqual(source.status.key,'closed')
    ...
    def test_cross_org_merge_is_rejected(self):
        ...
        with self.assertRaises(ValidationError):
            services.merge_tickets(actor=self.staff,primary_id=primary.pk,source_ids=[other.pk])

    def test_merge_does_not_collide_on_source_scoped_retry_keys(self):
        primary=self.create()
        source=self.create()
        key=uuid.uuid4()
        for t in (primary,source):
            services.reply(ticket_id=t.pk,actor=self.user,body='Message',client_key=key)
        services.merge_tickets(actor=self.staff,primary_id=primary.pk,source_ids=[source.pk])
        self.assertEqual(primary.messages.filter(body='Message').count(),2)

    def test_auto_close_exclusions_and_elapsed_hours(self):
        config=SupportSettings.load()
        config.auto_close_hours=24
        config.save()
        tickets={key:self.create(subject=key) for key in ('open','in_progress','answered','on_hold','closed')}
        for key,t in tickets.items():
            Ticket.objects.filter(pk=t.pk).update(status=TicketStatus.objects.get(key=key),last_public_activity_at=timezone.now()-timedelta(hours=25))
        self.assertEqual(maintain_tickets()['auto_closed'],2)
        for key,t in tickets.items():
            t.refresh_from_db()
            self.assertEqual(t.status.key,'closed' if key in ('open','answered') else key)

    def email(self, *, sender=None, subject='Email issue', message_id='one', body='Reproduce steps'):
        msg=EmailMessage()
        msg['From']=sender or self.user.email
        msg['To']='care@example.test'
        msg['Subject']=subject
        msg['Message-ID']=f'<{message_id}@example.test>'
    ...
    def test_verified_email_dedup_and_thread_reply(self):
        ...
        self.assertEqual(one.pk,two.pk)
        self.assertEqual(InboundReceipt.objects.count(),1)
    ...
    def test_email_mismatched_sender_and_other_org_reference(self):
        self.enable_mail()
        ticket=self.create()
        with self.assertRaises(RejectedEmail):
            ingest_verified_email(self.email().as_bytes(),verified_sender=self.outsider.email)
        msg=self.email(sender=self.outsider.email,subject=f'[{ticket.reference}]')
        with self.assertRaises(RejectedEmail):
            ingest_verified_email(msg.as_bytes(),verified_sender=self.outsider.email)
    ...
    def test_untrusted_authentication_results_fail_closed(self):
        msg=self.email()
        msg['Authentication-Results']='mx.example.test; dmarc=pass header.from=example.test'
        with self.assertRaises(RejectedEmail):
            trusted_imap_sender(msg)
    ...
    def test_wrong_area_and_org_admin_never_get_platform_access(self):
        ...
        with self.assertRaises(PermissionDenied):
            views.staff_list(request)
        request=self.request(user=self.staff)
        request.shvya_session_area='admin'
        with self.assertRaises(PermissionDenied):
            views.staff_list(request)
    ...
    def test_templates_compile_and_both_lists_render(self):
        ...
        for fn,user in ((views.customer_list,self.user),(views.staff_list,self.staff)):
            with self.subTest(user=user.email):
                response=fn(self.request(user=user))
                self.assertEqual(response.status_code,200)
