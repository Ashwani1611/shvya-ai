...
def hosted_session_chat_send_view(request, account_id):
    ...
    return JsonResponse(
        {"ok": True, "message": {"id": str(message.id), "body": message.body, "status": message.status}},
        status=201,
    )
...
def hosted_session_chat_media_send_view(request, account_id):
    ...
    if not normalized_chat:
        return JsonResponse({"ok": False, "error": "Invalid WhatsApp recipient."}, status=400)
    ...
    if not message_type:
        return JsonResponse({"ok": False, "error": "Choose photo, video, or document."}, status=400)
