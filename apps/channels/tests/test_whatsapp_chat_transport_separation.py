...
class WhatsAppChatTransportSeparationTests(TestCase):
    ...
    def test_api_send_never_selects_newer_hosted_account(self, delay):
        ...
        self.make_message(account=self.api_account, lead=lead, body="Older API")
        self.make_message(account=self.hosted_account, lead=lead, body="Newer Hosted")

        # Make the 24-hour window API-specific by adding a fresh API inbound row.
    ...
    @patch("apps.channels.hosted_send_ui.send_hosted_whatsapp_message_task.delay")
    def test_hosted_text_uses_hosted_task_not_meta_task(self, delay):
        lead = self.make_lead("Hosted Text Lead", "+919666666666")
        response = self.client.post(
            reverse("whatsapp-hosted-session-chat-send", args=[self.hosted_account.id]),
            data='{"chat":"+919666666666","body":"Hosted reply"}',
