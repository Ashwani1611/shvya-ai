...
def _resolve_state_before_response(*, organization, lead, source_message_id, decision) -> dict[str, Any]:
    ...
    The canonical task generates the customer-facing response only after this
    transaction commits, so that response generation sees the resulting CRM state.
    """
    ...
    with transaction.atomic():
        ...
        latest = _latest_inbound(locked_lead)
        ...
        # 1) Persist mapped CRM attributes first. These actions have already been
        # constrained to organization-owned definitions and evidence by the graph.
        ...
        # 2) Persist qualification answers/state. This remains independent of
        # reminder/workflow state and never derives completion from response text.
        ...
        # 3) Qualification completion owns the Qualified transition. If completion
        # is true, it takes priority over any generic model-proposed stage action.
        completion_stage = _qualified_action(
            lead=locked_lead,
            qualification_state=qualification_state,
        )
        stage_actions = [completion_stage] if completion_stage else proposed_stage_actions[:1]
        if stage_actions:
            results.extend(
                executor.execute(
                    organization=organization,
        ...
        # 4) Execute the remaining already-validated workflow actions only after
        # qualification/stage state is settled. Reminder behavior is preserved.
        if other_actions:
            results.extend(
                executor.execute(
                    organization=organization,
...
def _suppress_already_resolved_actions(current_builder):
    """Second/final response generation must not repeat pre-resolved CRM writes."""
...
def install_transactional_turn_runtime() -> None:
    """Resolve CRM state before the canonical task builds its final response."""
    global _INSTALLED
    if _INSTALLED:
        return
    ...
    def transactional_execute(*, task, lead_id: str):
        ...
        if _message_state_resolved(lead=lead, source_message_id=source.pk):
            return original_execute(task=task, lead_id=lead_id)
        ...
        try:
            ...
            # Preserve the existing retry/fail-soft behavior if the resolution
            # pass itself cannot produce a safe decision.
            logger.exception(
        ...
        if not _state_changing_decision(draft):
            # State is unchanged, so the already-validated draft is also a valid
            # response for the resulting state. Reuse it in the canonical task
            # rather than paying for an unnecessary second provider call.
            token = _PRECOMPUTED_DECISION.set(
                {
                    "lead_id": str(lead.pk),
        ...
        try:
            ...
            # Rollback keeps the old single-phase path safe and backward
            # compatible instead of partially applying a broken pre-resolution.
            logger.exception(
        ...
        # The canonical task now re-reads the lead and generates/validates the
        # customer response from the committed resulting CRM state. The action
        # builder suppresses writes already applied for this exact inbound ID.
        return original_execute(task=task, lead_id=lead_id)
